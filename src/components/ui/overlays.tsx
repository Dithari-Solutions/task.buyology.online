...
export function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-4 sm:p-6">
      <div className="fixed inset-0 animate-fade-in bg-ink-950/40 backdrop-blur-[2px]" onClick={onClose} />
      <div
        className={cn(
          'relative z-10 my-auto w-full animate-slide-up rounded-2xl bg-white shadow-pop',
          width,
        )}
        role="dialog"
        aria-modal="true"
      >
        ...
        {footer && <footer className="flex items-center justify-end gap-2 border-t border-ink-100 px-6 py-4">{footer}</footer>}
      </div>
    </div>
  );
}
...
export function Drawer({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      <div className="absolute inset-0 animate-fade-in bg-ink-950/40 backdrop-blur-[2px]" onClick={onClose} />
      <aside
        className={cn('relative z-10 flex h-full w-full animate-slide-in flex-col bg-white shadow-pop', width)}
        role="dialog"
        aria-modal="true"
      >
        {children}
