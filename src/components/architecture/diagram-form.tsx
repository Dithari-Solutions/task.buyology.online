...
import { useDirectory, usePlatforms, useSaveDiagram } from '@/lib/queries';
...
export function DiagramForm({
  ...
  useEffect(() => {
    ...
    setOwnerId(diagram?.owner ? String(diagram.owner.id) : '');
  ...
  return (
    ...
    >
      <div className="space-y-3.5">
        <Field label="Name">
          <Input value={name} onChange={(event) => setName(event.target.value)} placeholder="Student Portal architecture" />
        </Field>
