  ...
  return (
    <div className="flex h-full w-[302px] shrink-0 flex-col">
      <div className="mb-2.5 flex items-center gap-2 px-1">
        <h3 className="text-[12.5px] font-semibold uppercase tracking-[0.06em] text-ink-500">{column.name}</h3>
        <span
          className={cn(
            'rounded-md px-1.5 py-0.5 text-[11px] font-semibold',
            overLimit ? 'bg-rose-100 text-rose-600' : 'bg-ink-200/70 text-ink-600',
          )}
        >
          {column.tasks.length}
          {column.wipLimit != null && `/${column.wipLimit}`}
        </span>
        <span className={cn('rounded px-1.5 py-0.5 text-[10px] font-semibold', meta.color)}>{meta.label}</span>
        {onCreate && (
          <button
            onClick={() => onCreate(column.id)}
            className="ml-auto flex h-6 w-6 items-center justify-center rounded-md text-ink-400 transition-colors hover:bg-ink-200/60 hover:text-ink-700"
      ...
      <div
        ref={setNodeRef}
        className={cn(
          'flex min-h-[140px] flex-1 flex-col gap-2 overflow-y-auto rounded-xl border border-transparent bg-ink-100/60 p-2 transition-colors',
          isOver && 'border-brand-300 bg-brand-50/70',
          overLimit && 'border-rose-200 bg-rose-50/40',
        )}
      >
        ...
        {column.tasks.length === 0 && (
          <button
            onClick={() => onCreate?.(column.id)}
            className="flex h-20 items-center justify-center rounded-lg border border-dashed border-ink-300/80 text-[12.5px] text-ink-400 transition-colors hover:border-brand-300 hover:text-brand-600"
          >
            Drop a task here
          </button>
        )}
