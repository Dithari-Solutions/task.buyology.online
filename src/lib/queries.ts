...
export function useBoards(platformId?: number | null, mine = false) {
  return useQuery({
    queryKey: ['boards', platformId ?? null, mine],
    queryFn: async () =>
      (
        await api.get<BoardSummary[]>('/boards', {
          params: { platformId: platformId ?? undefined, mine: mine || undefined },
        })
      ).data,
  });
}
...
export function useAddBoardMembers(boardKey: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ boardId, userIds }: { boardId: number; userIds: number[] }) =>
      (await api.post<UserSummary[]>(`/boards/${boardId}/members`, { userIds })).data,
    onSuccess: () => {
...
export function useRemoveBoardMember(boardKey: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ boardId, userId }: { boardId: number; userId: number }) =>
...
export function useDirectory() {
  return useQuery({
    queryKey: ['directory'],
    queryFn: async () => (await api.get<UserSummary[]>('/users/directory')).data,
    staleTime: 5 * 60_000,
  });
}
...
export function useUsers(filters: UserFilters) {
  return useQuery({
    ...
    queryFn: async () =>
      (
        await api.get<PageResponse<User>>('/users', {
          params: {
            search: filters.search?.trim() || undefined,
...
export function useCreateUser() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (payload: UserPayload) => (await api.post<CreateUserResponse>('/users', payload)).data,
    onSuccess: () => {
...
export function useSetUserActive() {
  ...
  return useMutation({
    mutationFn: async ({ id, active }: { id: number; active: boolean }) =>
      (await api.patch<User>(`/users/${id}/status`, null, { params: { active } })).data,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['users'] });
      queryClient.invalidateQueries({ queryKey: ['directory'] });
    },
...
export function useResetUserPassword() {
  return useMutation({
    mutationFn: async (id: number) =>
      (await api.post<{ email: string; temporaryPassword: string; emailSent: boolean }>(`/users/${id}/reset-password`))
        .data,
  });
}
...
export function useUpdateProfile() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (payload: { fullName: string; title?: string; department?: string; phone?: string; emailNotifications?: boolean }) =>
