...
export const KIND_META: Record<ArchNodeKind, { label: string; color: string; accent: string }> = {
  COMPONENT: { label: 'Component', color: '#402f75', accent: 'bg-brand-50 text-brand-600' },
  SERVICE: { label: 'Service', color: '#0ea5e9', accent: 'bg-sky-50 text-sky-600' },
  DATABASE: { label: 'Database', color: '#14b8a6', accent: 'bg-teal-50 text-teal-600' },
  EXTERNAL: { label: 'External', color: '#f59e0b', accent: 'bg-amber-50 text-amber-600' },
  NOTE: { label: 'Note', color: '#ffbe12', accent: 'bg-accent-50 text-accent-900' },
};

export const STATUS_META: Record<DiagramStatus, { label: string; color: string }> = {
  DRAFT: { label: 'Draft', color: 'bg-ink-100 text-ink-600' },
  IN_REVIEW: { label: 'In review', color: 'bg-amber-50 text-amber-700' },
  APPROVED: { label: 'Approved', color: 'bg-emerald-50 text-emerald-700' },
  DEPRECATED: { label: 'Deprecated', color: 'bg-rose-50 text-rose-600' },
};
