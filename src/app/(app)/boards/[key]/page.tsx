...
import { Filter, Plus, Search, Settings2, X } from 'lucide-react';
...
import { AvatarGroup, Badge, Button, EmptyState, Input, PageLoader, Select } from '@/components/ui';
...
export default function BoardPage() {
  ...
  const hasFilters = Boolean(search || assigneeId || type || priority);
  ...
  return (
    <>
      <Topbar
        ...
        actions={
          <div className="flex items-center gap-2">
            {board && <AvatarGroup users={board.members} />}
            <Button
      ...
      {filtersOpen && (
        <div className="flex flex-wrap items-center gap-2 border-b border-ink-200/70 bg-white px-4 py-3 sm:px-6">
          ...
          <Select value={assigneeId} onChange={(event) => setAssigneeId(event.target.value)} className="h-9 w-auto min-w-[170px]">
      ...
      <div className={cn('flex-1 overflow-hidden px-4 py-5 sm:px-6')}>
        {isLoading || loadingBoard || !kanban ? (
          ...
          <EmptyState
            ...
            description="An administrator can add columns from the board settings."
          />
        ) : (
          <div className="h-full">
            {board?.description && (
              <p className="mb-4 max-w-3xl text-[13px] leading-relaxed text-ink-500">{board.description}</p>
            )}
            <KanbanBoardView
