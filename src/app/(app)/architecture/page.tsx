...
export default function ArchitecturePage() {
  ...
  return (
    <>
      <Topbar
        title="Architecture"
        subtitle="Models of how the university's systems fit together"
        actions={
          isAdmin ? (
        ...
        <div className="mx-auto max-w-[1400px] space-y-5">
          ...
          {isLoading ? (
            ...
            <EmptyState
              ...
              description={
                isAdmin
                  ? 'Create one to start documenting how a system is put together.'
                  : 'An administrator has not published any architecture models yet.'
              }
              action={
                isAdmin ? (
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
              {filtered.map((diagram) => (
                <Link key={diagram.id} href={`/architecture/${diagram.id}`}>
                  <Card className="group h-full overflow-hidden transition-all duration-200 hover:-translate-y-0.5 hover:shadow-pop">
                    <div className="h-1.5" style={{ backgroundColor: diagram.color ?? '#402f75' }} />
                    <div className="p-4">
                      <div className="flex items-start justify-between gap-2">
                        <h3 className="min-w-0 truncate text-[14.5px] font-semibold text-ink-900">{diagram.name}</h3>
                        <Badge className={STATUS_META[diagram.status].color}>{STATUS_META[diagram.status].label}</Badge>
                      </div>

                      {diagram.platformName && (
                        <p className="mt-1 text-[12px] text-ink-500">{diagram.platformName}</p>
                      )}
                      {diagram.description && (
                      ...
                      <div className="mt-4 flex items-center justify-between text-[12px] text-ink-500">
                        <span>{diagram.nodeCount} components</span>
