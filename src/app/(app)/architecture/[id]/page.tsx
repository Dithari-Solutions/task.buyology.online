...
export default function DiagramPage() {
  ...
  }, [isAdmin, selectedNode, selectedEdge]);
  ...
  const showInspector = isAdmin && (selectedNode || selectedEdge);
  ...
  return (
    <>
      <Topbar
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1 rounded-lg border border-ink-200 px-1">
              <button
                onClick={() => setZoom(ZOOM_STEPS[Math.max(0, zoomIndex - 1)])}
                disabled={zoomIndex <= 0}
                className="rounded p-1 text-ink-500 disabled:opacity-30"
                aria-label="Zoom out"
              >
                <Minus className="h-3.5 w-3.5" />
              </button>
              <span className="w-10 text-center text-[11.5px] tabular-nums text-ink-600">
                {Math.round(zoom * 100)}%
              </span>
              <button
      ...
      {isAdmin && (
        <div className="arch-toolbar flex flex-wrap items-center gap-2 border-b border-ink-100 bg-white px-4 py-2 sm:px-6">
          ...
          {connectFrom !== null && (
            <span className="ml-2 flex items-center gap-2 rounded-lg bg-brand-50 px-2.5 py-1 text-[12px] font-medium text-brand-700">
              Pick the component to connect to
              <button onClick={() => setConnectFrom(null)} className="text-brand-500 hover:text-brand-700">
                Cancel
              </button>
            </span>
          )}
      ...
      <div className="flex min-h-0 flex-1">
        <div className="arch-canvas min-w-0 flex-1 overflow-auto bg-ink-50/40">
          <ArchitectureCanvas
            nodes={nodes}
            edges={edges}
            zoom={zoom}
        ...
        {showInspector && (
          <aside className="arch-inspector w-[300px] shrink-0 overflow-y-auto border-l border-ink-100 bg-white p-4">
            <h2 className="mb-3 text-[13px] font-semibold text-ink-900">
