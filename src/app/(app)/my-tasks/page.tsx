...
function MyTasksContent() {
  ...
  useEffect(() => {
    ...
    if (queryScope === 'all') setScope('all');
  }, [searchParams]);
  ...
  return (
    <>
      <Topbar
        ...
        subtitle={
          data ? `${data.totalElements} task${data.totalElements === 1 ? '' : 's'} match your filters` : undefined
        }
        actions={
          <div className="hidden items-center gap-1 rounded-lg bg-ink-100 p-0.5 sm:flex">
        ...
        <div className="mx-auto max-w-[1200px] space-y-5">
          ...
          {isLoading ? (
            ...
            <EmptyState
              ...
              description={
                scope === 'mine'
                  ? 'No tasks are assigned to you with these filters.'
                ...
                {data.content.map((task) => (
                  <li key={task.id}>
                    <button
                      ...
                      className="flex w-full items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-ink-50/60 sm:px-5"
                    >
                      <span
                        className={cn(
                      ...
                      <span className="hidden w-20 shrink-0 font-mono text-[11.5px] font-semibold text-ink-400 sm:block">
                        {task.taskKey}
                      </span>
                      <span className="min-w-0 flex-1">
                        <span className="block truncate text-[13.5px] font-medium text-ink-900">{task.title}</span>
                        <span className="mt-0.5 block text-[11.5px] text-ink-400">
                          Updated {fromNow(task.updatedAt)}
                        </span>
                      </span>
                      <Badge className={CATEGORY_META[task.category].color}>
                      ...
                      <span className={cn('h-1.5 w-1.5 shrink-0 rounded-full', PRIORITY_META[task.priority].dot)} />
